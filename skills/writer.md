...
**Cookie banners and modals are unacceptable in screenshots.** Before capturing any web page, dismiss the cookie-consent banner, GDPR modal, newsletter signup popup or any other overlay. Use a Playwright script that clicks "Accept" / "OK" on common patterns and removes residual fixed-position overlays before the screenshot fires. A reference script lives at `scripts/screenshot-clean.mjs` in the reference customer; copy it into your project and use it for every web capture. After capture, eyeball the screenshot — if there is a cookie banner, GDPR modal, fixed-position overlay or sticky nav element that doesn't belong, re-capture. Shipping a screenshot with a cookie banner is a draft failure that gets caught in review.
